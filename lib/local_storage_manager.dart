...
class LocalStorageManager extends StatefulWidget {
  const LocalStorageManager({Key? key}) : super(key: key);

  @override
  _LocalStorageManager createState() => _LocalStorageManager();
...
class _LocalStorageManager extends State<LocalStorageManager> {
  final String jsonKey = 'default_data_json_key';
  Map<String, dynamic> jsonData = {}; // Initialize with your default value
  ...
  Future<void> _loadJsonData() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final String? jsonString = prefs.getString(jsonKey);
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            Text('Stored JSON Data: $jsonData'),
            ElevatedButton(
              onPressed: () async {
                ...
                await _saveJsonData(sampleJson);

                // Reload the JSON data
                await _loadJsonData();
              },
              child: Text('Store JSON Data Locally'),
            ),
          ],
        ),
      ),
